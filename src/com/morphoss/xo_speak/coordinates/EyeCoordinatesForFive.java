...
public 	class EyeCoordinatesForFive extends EyeInitCoordinates {
	...
	public void calc(ArrayList<EyeOutside> eyes, int canvasWidth, int canvasHeight) {
		...
		eyes.get(0).radius /= 1.1;
		eyes.get(1).centerX = (int) (w-1.3*h);
		eyes.get(1).radius /= 1.1;
		eyes.get(2).centerX = w;
		eyes.get(2).radius /= 1.1;
		eyes.get(3).centerX = (int) (w+1.3*h);
		eyes.get(3).radius /= 1.1;
		eyes.get(4).centerX = (int) (w+2.5*h);
		eyes.get(4).radius /= 1.1;
	}
}
